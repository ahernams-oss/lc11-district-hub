...
import { Plus, Pencil, Trash2, CheckCircle2, XCircle, Download, Tag, Paperclip } from "lucide-react";
...
type Conta = Awaited<ReturnType<typeof listContasPagar>>[number];
...
function ContasPagarPage() {
  ...
  const { data: contas, isLoading } = useQuery({
    queryKey: ["contas-pagar", mes, statusFilter],
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => {
      setMsg({ type: "ok", text: "Conta salva com sucesso." });
      setDrawer(false);
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.valor || !form.descricao || !form.vencimento) return;
    saveMut.mutate(form);
  }

  const catsDespesa = (categorias ?? []).filter((c: any) => c.tipo === "despesa");
  const total = (contas ?? []).reduce((s, c) => s + c.valor, 0);
  const pago = (contas ?? []).filter((c) => c.status === "pago").reduce((s, c) => s + ((c as any).valor_pago ?? c.valor), 0);
  ...
  return (
    <div>
      <GestaoHeader
        ...
        subtitle="Controle de obrigações e despesas do distrito"
        breadcrumbs={[{ label: "Gestão", to: "/gestao" }, { label: "Financeiro", to: "/gestao/financeiro" }, { label: "Contas a Pagar" }]}
        actions={
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="rounded-xl border border-white/8 bg-white/[0.03] p-4">
            <div className="text-xs text-slate-500 uppercase tracking-wider">Total no período</div>
            <div className="mt-1 text-xl font-bold text-white">{formatBRL(total)}</div>
          </div>
          <div className="rounded-xl border border-white/8 bg-white/[0.03] p-4">
            <div className="text-xs text-slate-500 uppercase tracking-wider">Pago</div>
            <div className="mt-1 text-xl font-bold text-emerald-400">{formatBRL(pago)}</div>
          </div>
          <div className="rounded-xl border border-white/8 bg-white/[0.03] p-4">
      ...
      {confirmDelete && (
        <>
          ...
          <div className="fixed left-1/2 top-1/2 z-50 w-[360px] -translate-x-1/2 -translate-y-1/2 rounded-xl border border-white/10 bg-[#0d1321] p-6 shadow-2xl">
            ...
            <p className="mt-2 text-sm text-slate-400">Tem certeza que deseja excluir este registro? Esta ação não pode ser desfeita.</p>
      ...
      {/* Form Drawer */}
      <Drawer open={drawer} onClose={() => setDrawer(false)} title={form.id ? "Editar Conta a Pagar" : "Nova Conta a Pagar"}>
        <form onSubmit={handleSubmit} className="space-y-5">
          <Field label="Descrição" required>
            <FormInput
              value={form.descricao}
              onChange={(e) => setForm((f) => ({ ...f, descricao: e.target.value }))}
              placeholder="Ex: Aluguel sede, serviço gráfico..."
              required
            />
          </Field>
          ...
          <FormRow>
            ...
            <Field label="Conta Bancária">
              <FormSelect
                value={form.conta_id ?? ""}
                onChange={(e) => setForm((f) => ({ ...f, conta_id: e.target.value || undefined }))}
              >
          ...
          {(form.status as string) === "pago" && (
            <FormRow>
              ...
              <Field label="Valor pago">
                <CurrencyInput value={form.valor_pago ?? 0} onChange={(v) => setForm((f) => ({ ...f, valor_pago: v }))} />
              </Field>
            </FormRow>
          )}
          ...
          <Field label="Documento (NF, recibo)">
            <FormInput
              ...
              placeholder="Número ou código do documento"
            />
          </Field>
